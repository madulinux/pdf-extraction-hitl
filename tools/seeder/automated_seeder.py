...
class AutomatedSeeder:
    ...
    def __init__(self, api_base_url: str = API_BASE_URL, username: str = 'admin', password: str = 'admin123'):
    ...
    def submit_corrections(self, document_id: int, corrections: Dict) -> Dict:
        ...
        logger.info(f"📝 Submitting {len(corrections)} corrections for document {document_id}")
        
        url = f"{self.api_base_url}/extraction/validate"
        
        response = self.session.post(url, json={'corrections': corrections, 'document_id': document_id})
        response.raise_for_status()
    ...
    def process_single_document(
        ...
        try:
            ...
            return {
                ...
                'accuracy': (len(ground_truth) - len(corrections)) / len(ground_truth) if ground_truth else 0,
    ...
    def seed_template(
        ...
        if len(pdf_files) < count:
            logger.warning(f"⚠️  Only found {len(pdf_files)} documents, expected {count}")
            logger.warning(f"    Please run: python ../cmd/main.py generate-documents --count {count}")
        ...
        logger.info(f"  Incorrect: {incorrect_fields} ({incorrect_fields/total_fields*100:.1f}%)")
...
def main():
    ...
    parser.add_argument('--template', type=str, required=True, 
                       choices=['certificate_template', 'contract_template', 'invoice_template', 
                               'job_application_template', 'medical_form_template'],
