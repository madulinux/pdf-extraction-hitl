...
TEMPLATE_CONFIGS = {
    "letter_template": {"name": "Surat", "default_count": 10, "priority": 1},
    "table_template": {"name": "Invoice", "default_count": 10, "priority": 2},
    "form_template": {"name": "Formulir", "default_count": 10, "priority": 3},
    "mixed_template": {"name": "Laporan", "default_count": 10, "priority": 4},
    "certificate_template": {"name": "Sertifikat", "default_count": 10, "priority": 5},
}
...
class BatchSeeder:
    ...
    def generate_documents(
        self, template_type: str, count: int, use_word: bool = False
    ) -> bool:
        """
    ...
    def train_model(self, template_name: str):
        """
        Train model
        """
        logger.info("Training model...")
        self.seeder.train_model(template_name, full=True)

    def seed_template(self, template_type: str, count: int = None, retrain: bool = False) -> Dict:
        """
        ...
        Args:
            ...
            retrain: Whether to retrain the model after seeding
        ...
        result = self.seeder.seed_template(
    ...
    def seed_all(
        self, count: int = None, generate: bool = False, use_word: bool = False
    ):
        """
        Seed all templates
        ...
        templates = sorted(TEMPLATE_CONFIGS.items(), key=lambda x: x[1]["priority"])
        ...
        for template_type, config in templates:
            ...
            if generate:
                ...
                if not success:
                    logger.warning(
                        f"⚠️  Skipping {template_type} due to generation failure"
                    )
                    continue
    ...
    def print_summary(self, total_time: float):
        ...
        if successful:
            ...
            if total_fields > 0:
                ...
                logger.info(
                    f"Incorrect: {total_fields - correct_fields} ({(total_fields-correct_fields)/total_fields*100:.1f}%)"
                )
            else:
        ...
        if failed:
            ...
            for result in failed:
                logger.info(
                    f"  - {result.get('template_type', 'unknown')}: {result.get('error')}"
                )

        logger.info(f"\n⏱️  Total time: {total_time:.2f} seconds")
        logger.info(
            f"⏱️  Avg time per template: {total_time/len(self.results):.2f} seconds"
        )
        logger.info("=" * 70)
...
def main():
    ...
    parser.add_argument("--all", action="store_true", help="Seed all templates")
    parser.add_argument(
        "--templates",
        nargs="+",
    ...
    parser.add_argument(
        "--count", type=int, help="Override document count for all templates"
    )
    parser.add_argument(
        "--generate",
        action="store_true",
    ...
    parser.add_argument(
        "--train", action="store_true", help="Train model after seeding"
    )

    parser.add_argument(
        "--retrain", action="store_true", help="Train model after seeding"
    )
    ...
    if args.all:
        batch_seeder.seed_all(
            count=args.count, generate=args.generate, use_word=args.use_word
        )
    else:
        for template in args.templates:
            ...
            batch_seeder.seed_template(template, args.count, args.retrain)
            if args.train:
                time.sleep(1)
                batch_seeder.train_model(template)
