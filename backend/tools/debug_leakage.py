...
validated_docs = document_repo.find_validated_documents(template_id=1)
...
X_train_split, X_test_split, ids_train, ids_test = train_test_split(
    X_train, doc_ids, test_size=0.2, random_state=42
)
...
for i, content in enumerate(X_train_split):
    ...
    if h in train_hashes:
        print(
            f"   ⚠️  Duplicate in TRAIN: doc {ids_train[i]} == doc {ids_train[train_hashes[h]]}"
        )
    train_hashes[h] = i
...
for i, test_content in enumerate(X_test_split):
    ...
    if test_hash in train_hashes:
        ...
        print(
            f"   ❌ LEAKAGE: test doc {ids_test[i]} == train doc {ids_train[train_idx]}"
        )
        duplicates_found += 1
