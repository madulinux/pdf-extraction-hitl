...
def trace_extraction(document_id: int):
    ...
    template = db.get_template(doc['template_id'])
...
def trace_model_prediction(document_id: int):
    ...
    learner = AdaptiveLearner()
    features = [learner._extract_word_features(words[i], words, i) for i in range(len(words))]
...
def full_pipeline_trace(document_id: int):
    ...
    if training_result:
        print(f"✅ Training Data: {training_result['total_tokens']} tokens, {len(training_result['label_counts'])} unique labels")
    
    if model_result:
        non_o = sum(count for label, count in model_result['pred_counts'].items() if label != 'O')
        print(f"✅ Model Prediction: {non_o} tokens labeled (out of {sum(model_result['pred_counts'].values())})")
    
    if comparison_result:
        print(f"✅ Accuracy: {comparison_result['accuracy']:.2f}% ({comparison_result['correct']}/{comparison_result['total']})")
    
    print("\n" + "="*80)

def main():
    parser = argparse.ArgumentParser(description='Diagnostic tool for extraction pipeline')
