...
from .strategies import ExtractionStrategy, FieldValue
from .crf_strategy import CRFExtractionStrategy
...
class HybridExtractionStrategy:
    ...
    def extract_all_fields(
        ...
        if model_path:
            if os.path.exists(model_path):
                ...
                print(
                    f"✅ [HybridStrategy] CRF strategy initialized with adaptive weight {crf_weight:.2f}"
                )
            else:
        ...
        for field_name, field_config in fields.items():
            ...
            if extraction_result:
                ...
                results["confidence_scores"][field_name] = extraction_result[
                    "confidence"
                ]
                ...
                if "metadata" in extraction_result:
                    ...
                    if "all_strategies_attempted" in metadata:
                        strategy_info["all_strategies_attempted"] = metadata[
        ...
        self.logger.info(
            f"Extraction completed: {len(results['extracted_data'])} fields in {extraction_time_ms}ms"
        )
        if results["conflicts"]:
            self.logger.info(
                f"Conflicts detected in {len(results['conflicts'])} fields"
            )

        return results

    def _analyze_context(
        self, template_config: Dict[str, Any], model_path: Optional[str]
    ) -> ExtractionContext:
        ...
        return ExtractionContext(
            ...
            has_model=(
                model_path is not None and os.path.exists(model_path)
                if model_path
                else False
            ),
    ...
    def _extract_field_with_conflict_detection(
        ...
        for location_idx, location in enumerate(locations):
            ...
            if final_result:
                ...
                if "all_strategies_attempted" in final_result.metadata:
                    result_dict["all_strategies_attempted"] = final_result.metadata[
                        "all_strategies_attempted"
                    ]
        ...
        if "all_strategies_attempted" in best_result:
            metadata["all_strategies_attempted"] = best_result[
                "all_strategies_attempted"
            ]
    ...
    def _extract_field_with_strategies(
        ...
        self.logger.debug(
            f"🔍 Extracting field '{field_name}' with multiple strategies"
        )

        # Always try rule-based (baseline)
        try:
            rule_result = self.rule_based_strategy.extract(
                pdf_path, field_config, all_words
            )
            results[StrategyType.RULE_BASED] = rule_result
            if rule_result:
                self.logger.debug(
                    f"  ✅ Rule-based: {rule_result.value[:50]}... (conf: {rule_result.confidence})"
                )
        ...
        if self.crf_strategy:
            ...
            try:
                crf_result = self.crf_strategy.extract(
                    pdf_path, field_config, all_words
                )
                ...
                if crf_result:
                    self.logger.info(
                        f"  ✅ CRF: {crf_result.value[:50]}... (conf: {crf_result.confidence:.4f})"
                    )
                else:
                    # ✅ Log when CRF returns None (important for debugging)
                    self.logger.warning(
                        f"  ⚠️ CRF returned None for '{field_name}' - model may not have learned this field yet"
                    )
            except Exception as e:
    ...
    def _combine_strategy_results(
        ...
        field_performance = self._get_field_performance_from_db(
            context.template_id, field_name
        )
        ...
        for st, fv in strategy_results.items():
            ...
            perf_data = field_performance.get(
                st.value, {"accuracy": 0.5, "attempts": 0}
            )
            hist_accuracy = (
                perf_data.get("accuracy", 0.5)
                if isinstance(perf_data, dict)
                else perf_data
            )
            hist_attempts = (
                perf_data.get("attempts", 0) if isinstance(perf_data, dict) else 0
            )

            # ✅ ADAPTIVE THRESHOLD based on historical performance (CONFIG-DRIVEN)
            min_confidence = self._get_confidence_threshold(
                hist_accuracy, hist_attempts
            )
        ...
        if not valid_results:
            self.logger.warning(
                f"  ⚠️ No strategies met confidence threshold for '{field_name}'"
            )

            # Count how many strategies returned results
            non_none_results = sum(
                1 for fv in strategy_results.values() if fv is not None
            )
            self.logger.info(
                f"  📊 Strategy results: {non_none_results}/{len(strategy_results)} strategies returned results"
            )
            ...
            for strategy_type, field_value in strategy_results.items():
                ...
                if (
                    field_value
                    and field_value.confidence > 0
                    and field_value.confidence > best_confidence
                ):
                    best_result = field_value
                    best_confidence = field_value.confidence
                    best_strategy = strategy_type
                    self.logger.debug(
                        f"    🔍 Found candidate: {strategy_type.value} (conf={field_value.confidence:.2f}, value='{field_value.value}')"
                    )

            # If we found any result, use it as fallback
            if best_result:
                self.logger.info(
                    f"  🔄 Using fallback: {best_strategy.value} (confidence: {best_confidence:.2f}, value: '{best_result.value}')"
                )
            ...
            self.logger.warning(
                f"  ⚠️ No fallback available for '{field_name}' - returning method='none'"
            )
            from .strategies import FieldValue
        ...
        for strategy_type, field_value in valid_results:
            ...
            perf_data = field_performance.get(
                strategy_type.value, {"accuracy": 0.5, "attempts": 0}
            )
        ...
        self.logger.info(
            f"  🏆 Winner: {best_strategy.value} (score: {best_score:.4f})"
        )
        ...
        best_result.method = (
            best_strategy.value
        )  # ✅ Direct strategy name (no 'hybrid-' prefix)
        ...
        best_result.metadata["selected_by"] = (
            "hybrid_strategy"  # ✅ Track that hybrid made the selection
        )
    ...
    def learn_from_feedback(
        ...
        strategies_used = extraction_results.get("metadata", {}).get(
            "strategies_used", []
        )

        for strategy_info in strategies_used:
            field_name = strategy_info.get(
                "field_name"
            )  # ✅ FIXED: use 'field_name' not 'field'
            ...
            if not field_name:
                self.logger.warning(
                    f"⚠️ Skipping strategy_info with missing field_name: {strategy_info}"
                )
                continue
                ...
                if strategy_name != method:  # Don't double-count the selected strategy
                    ...
                    if field_name in corrections:
                        ...
                        if strategy_was_correct:
                            self.logger.info(
                                f"    ✅ {strategy_name} had CORRECT value for '{field_name}'"
                            )
                        else:
                            self.logger.debug(
                                f"    ❌ {strategy_name} had WRONG value for '{field_name}'"
                            )
                            self.logger.debug(
                                f"       Strategy: '{strategy_value_norm[:50]}'"
                            )
                            self.logger.debug(
                                f"       Corrected: '{corrected_value_norm[:50]}'"
                            )
                    else:
                        # Field was NOT corrected - selected strategy was correct
                        # Check if this strategy also had the same (correct) value
                        selected_value = extraction_results.get(
                            "extracted_data", {}
                        ).get(field_name, "")
        ...
        self.logger.info(
            f"Feedback learning completed. Overall accuracy: {accuracy:.2%}"
        )
        ...
        if template_id:
            ...
            return {
                ...
                "total_extractions": sum(
                    sum(
                        perf.get("extraction_count", 0)
                        for perf in template_perf.values()
                    )
        ...
        try:
            ...
            if row and row["avg_accuracy"] is not None and row["total_fields"] > 0:
                ...
                confidence_factor = min(
                    1.0, total_fields / 10.0
                )  # Max confidence at 10+ fields
                base_weight = 0.3 + (avg_accuracy * 0.6)  # Scale 0.3-0.9
                adaptive_weight = base_weight * confidence_factor + 0.5 * (
                    1 - confidence_factor
                )
    ...
    def _get_confidence_threshold(self, accuracy: float, attempts: int) -> float:
        ...
        for level, config in self.CONFIDENCE_THRESHOLDS.items():
            if (
                ...
                and attempts >= config["min_attempts"]
            ):
    ...
    def _get_field_performance_from_db(
        self, template_id: int, field_name: str
    ) -> Dict[str, Dict]:
        """
    ...
    def _load_performance_history(self) -> None:
        ...
        try:
            if os.path.exists(self.performance_file):
                ...
                self.logger.info(
                    f"Loaded performance history from {self.performance_file}"
                )
        except Exception as e:
