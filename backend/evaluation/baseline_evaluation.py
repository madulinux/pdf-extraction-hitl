...
class BaselineEvaluator:
    ...
    def is_match(
        self, extracted: str, ground_truth: str, use_fuzzy: bool = True
    ) -> tuple:
        """
    ...
    def evaluate_overall_performance(self) -> Dict:
        ...
        documents = self.document_repo.find_validated_documents(self.template_id)

        if not documents:
            print("❌ No validated documents found!")
            print("   Tip: Make sure you have documents with status='validated'")
            print(
                "   Check: SELECT * FROM documents WHERE template_id=1 AND status='validated';"
            )
            return {
                "total_documents": 0,
                "total_fields": 0,
        ...
        for doc in documents:
            ...
            feedback_list = self.feedback_repo.find_by_document_id(doc["id"])
            corrections = {
            ...
            for field_name, extracted_value in extracted_data.items():
                ...
                # Use fuzzy matching
                is_correct, similarity = self.is_match(
                    extracted_value, ground_truth_value, use_fuzzy=True
                )
        ...
        print(
            f"   Avg Similarity: {results['avg_similarity']:.4f} ({results['avg_similarity']*100:.2f}%)"
        )
        print(
    ...
    def compare_strategies(self) -> pd.DataFrame:
        ...
        strategies = {"Hybrid (Current)": self.evaluate_overall_performance()}
    ...
    def run_full_evaluation(self):
        ...
        summary = {
            ...
            "per_field_metrics": (
                per_field_results.to_dict("records")
                if not per_field_results.empty
                else []
            ),
            "strategy_comparison": (
                strategy_comparison.to_dict("records")
