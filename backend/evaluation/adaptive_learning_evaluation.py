...
class AdaptiveLearningEvaluator:
    ...
    def simulate_incremental_learning(self, step_size: int = 5) -> pd.DataFrame:
        ...
        all_feedback = self.feedback_repo.find_by_document_id(self.template_id)
    ...
    def _evaluate_on_documents(self, doc_ids: List[int]) -> tuple:
        ...
        for doc_id in doc_ids:
            ...
            # Get ground truth
            feedback_list = self.feedback_repo.find_by_document_id(doc_id)
            ground_truth = {
                fb["field_name"]: fb["corrected_value"] for fb in feedback_list
            }
            ...
            for field_name in extracted_data.keys():
                ...
                truth = ground_truth.get(
                    field_name, pred
                )  # Use prediction if no ground truth
    ...
    def _plot_learning_curve(self, df: pd.DataFrame):
        ...
        ax1.set_title(
            "Learning Curve - Accuracy Improvement", fontsize=14, fontweight="bold"
        )
        ...
        ax2.bar(
            df["num_training_docs"], df["improvement"], alpha=0.7, color="steelblue"
        )
        ax2.set_xlabel("Number of Training Documents", fontsize=12)
        ax2.set_ylabel("Accuracy Improvement (Δ)", fontsize=12)
        ax2.set_title(
            "Incremental Improvement per Iteration", fontsize=14, fontweight="bold"
        )
    ...
    def _plot_training_mode_comparison(self, df: pd.DataFrame):
        ...
        accuracy_bars = ax.bar(
            x - width / 2, df["accuracy"], width, label="Accuracy", alpha=0.8
        )
        f1_bars = ax.bar(
            x + width / 2, df["f1_score"], width, label="F1-Score", alpha=0.8
        )

        ax.set_xlabel("Training Mode", fontsize=12)
        ax.set_ylabel("Score", fontsize=12)
        ax.set_title(
            "Training Mode Comparison - Performance Metrics",
            fontsize=14,
            fontweight="bold",
        )
        ax.set_xticks(x)
    ...
    def analyze_convergence(self, df: pd.DataFrame) -> Dict:
        ...
        if convergence_iteration:
            ...
            print(
                f"   Accuracy: {convergence_accuracy:.4f} ({convergence_accuracy*100:.2f}%)"
            )
        ...
        analysis = {
            ...
            "convergence_accuracy": (
                convergence_accuracy if convergence_iteration else None
            ),
    ...
    def run_full_evaluation(self):
        ...
        summary = {
            "learning_curve": (
                learning_curve_df.to_dict("records")
                if not learning_curve_df.empty
                else []
            ),
            "convergence_analysis": convergence_analysis,
            "training_mode_comparison": (
                training_mode_comparison.to_dict("records")
                if not training_mode_comparison.empty
                else []
            ),
        }
