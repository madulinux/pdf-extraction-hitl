...
def debug_training(template_id: int):
    ...
    # Get feedback
    db = DatabaseManager()
    feedback_list = db.get_feedback_for_training(template_id, unused_only=False)
    ...
    # Get document
    document = db.get_document(first_doc_id)
    if not document:
    ...
    print()
    print(f"Accuracy on training sample: {matches}/{len(labels)} = {matches/len(labels)*100:.1f}%")
    print()
